...
namespace CDGBulgariaTests.Service
{
	public class DiseasesServiceTests
	{
		...
		private List<CDGDisease> GetInitialData()
		{
			return new List<CDGDisease> {
				new CDGDisease {
					...
					CDGDiseaseType=new CDGDiseaseType
					{ Name="N-Type"} 
				  },
				new CDGDisease {
					Name=" MPMD2",
		...
		[Fact]
		public async Task GetAllCDGDiseaseTypes_WithInitialData_ShouldReturnCorrectResults()
		{
			...
			List<CDGDiseaseTypeServiceModel> expectedResults = GetInitialData().Select(disease => disease.CDGDiseaseType).To<CDGDiseaseTypeServiceModel>().ToList();
